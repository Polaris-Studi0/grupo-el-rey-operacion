-- Keep checkout history through fulfillment; expire only after 24 quiet hours.
create or replace function public.close_completed_whatsapp_conversations()
returns integer language plpgsql security definer set search_path = '' as $$
declare closed_count integer;
begin
  update public.whatsapp_conversations c set status='closed',closed_at=now()
  where c.status<>'closed' and c.source<>'internal_admin'
    and exists(select 1 from public.orders o where o.whatsapp_conversation_id=c.id)
    and not exists(select 1 from public.orders o where o.whatsapp_conversation_id=c.id and o.status not in ('delivered','cancelled'))
    and not exists(select 1 from public.human_tasks t where t.conversation_id=c.id and t.status in ('pending','in_progress'))
    and greatest(
      (select max(case when o.status='delivered' then coalesce(o.delivered_at,o.updated_at) else o.updated_at end) from public.orders o where o.whatsapp_conversation_id=c.id),
...
$$;
revoke all on function public.close_completed_whatsapp_conversations() from public,anon,authenticated;
grant execute on function public.close_completed_whatsapp_conversations() to service_role;
...
begin
  ...
  perform public.close_completed_whatsapp_conversations();

  select * into conversation from public.whatsapp_conversations
  where contact_id=contact.id
    and status in ('open','waiting_customer','waiting_human','converted')
    and source<>'internal_admin'
  order by created_at desc limit 1 for update;

  if conversation.id is null then
    select exists(
      select 1 from public.privacy_consents pc
      where pc.contact_id=contact.id and pc.granted
    ) into has_consent;

    update public.whatsapp_conversations
    set status='closed',closed_at=coalesce(closed_at,now())
    where contact_id=contact.id and source='internal_admin'
      and status in ('open','waiting_customer','waiting_human');

    insert into public.whatsapp_conversations(
      contact_id,branch_id,source,status,consent_status,consent_version,consented_at
    ) values(
      contact.id,p_branch_id,coalesce(nullif(trim(p_source),''),'whatsapp'),'open',
      case when has_consent then 'granted' else 'pending' end,
      case when has_consent then '2026-09-01' else null end,
...
-- Existing minute recovery tick also expires completed conversations, without n8n runs.
