> DOCUMENTO HISTÓRICO. Puede contener estados, accesos e instrucciones superados. No usar para activar ni desplegar. Leer primero ../../HANDOFF_N8N_WHATSAPP.md y ../ACCESOS.md.
...
Recuperación REAL comprobada: Cloudflare recuperó automáticamente el evento perdido en el segundo intento; 01/03 procesaron la consulta original (03: ejecución 1324). Respuesta: «Puedo consultar disponibilidad y precios de Amor y amistad. ¿En cuál sede o zona deseas que lo revise?». WhatsApp la entregó y el estado final es `read`, exactamente una copia (comprobación 1327). Inbox con `processed_at` presente y sin error (1326). La recuperación manual preparada en 1325 no reclamó nada ni envió mensajes porque el cron ya lo había hecho. Los cuatro nodos temporales adicionales se retiraron al terminar.

### Sedes y elección natural de opciones confirmadas — 2026-09-10

03 publicado `cf8d8237-b4ae-4246-af2b-69cb00ff3a75`, 13 nodos, parámetros leídos y verificados antes de publicar. La IA de las ejecuciones 1329/1331 había generado la lista correcta de diez sedes; la regla de product_discovery la sobrescribía. `response_topic` ahora separa navegación/información de la cotización. La lista usa branches reales y conserva estado y sede; la selección resuelta de Aures tiene prioridad sobre una clasificación atrasada de branch_list.

Las ejecuciones 1337/1339 demostraron otro error: al elegir «quiero un peluche», el cambio de interés desde Amor y Amistad provocaba un lookup nuevo y el modelo no agregaba una unidad. Se introduce memoria `sales_state.offered_products` separada del carrito: cada opción conserva tarea origen, sede, nombre, precio, unidades confirmadas y cita de la resolución. `product_offers` admite extracción semántica validada contra la respuesta real; existe recuperación de respuestas previas con varios productos. No se acepta una cita inventada ni se cruza el precio de las rosas con el del peluche. Una nueva cotización de la misma opción reemplaza el precio anterior.

`selection` permite interpretar referencias naturales con la IA (nombre abreviado, el primero, el más barato); «un/una/uno» es una unidad. La opción elegida avanza al siguiente dato faltante sin repetir opciones ni existencia ya confirmada. Si pide más unidades que las respaldadas, consulta esa cantidad. `sales_state` devuelve únicamente novedades/correcciones; el sistema conserva los datos previos. Las comprobaciones de pago y creación de pedido siguen independientes de la interpretación lingüística.
